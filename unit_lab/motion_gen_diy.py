...
class CuroboMotion:
    def __init__(self):
        ...
        # self.my_world.stage.SetDefaultPrim(self.my_world.stage.GetPrimAtPath("/World"))
        stage = self.my_world.stage
        ...
        self.robot, self.robot_prim_path = add_robot_to_scene(robot_cfg, self.my_world, position=np.array([0, 0, 0.03]))
    ...
    def update_lock_joints(self, locked_joints):
        if (
            ...
            after = time.time()
            # print("update lock joints time is {}".format(after - before))
        else:
            print("lock joints is the same, no need to update")


    def set_locked_joint_positions(self, first_time=False):
        if not self.lock_joints:
            return

        joint_positions = self.robot.get_joint_positions()
        ids = {}
        for idx in range(len(self.robot.dof_names)):
            name = self.robot.dof_names[idx]
            if name in self.lock_js_names:
                if first_time:
                    ids[name] = float(self.lock_joints[name])
                else:
                    ids[name] = float(joint_positions[idx])
        self.update_lock_joints(ids)

    
    def apply_lock_action(self):
        # 每帧重新设置需要锁定 joint 的目标
        action = ArticulationAction(
            joint_positions = np.array(lock_position_list),
            joint_indices = np.array(lock_index_list)
        )
    ...
    def run(self):
        ...
        # self.set_locked_joint_positions(first_time=True)
        ...
        while simulation_app.is_running():
            ...
            all_js_names = self.robot.dof_names
            sim_js_names = []
            lock_idx = []
            sim_js_positions = []
            sim_js_velocities = []
