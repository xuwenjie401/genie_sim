...
### Current Task
1. now free_worker_vertical.py is just a copy of free_worker_gripper.py, but it's created to do this task: 
...
4) This free_worker_vertical.py is a unit test, so "obstacle1-bottle" and "obstacle2-box" in the current code can remain as collision obstacles. However, for the grasp target, you could use a collision-free cube so that I can drag it around easily in Isaac Sim.
The current state machine and pause/reset rules may also be unnecessary. You may want to refer to free_worker_cube.py as well, since it is more relevant to this task.
